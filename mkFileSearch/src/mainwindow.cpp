...
#ifdef Q_OS_LINUX
    Qt::ColorScheme globalColorScheme = QGuiApplication::styleHints()->colorScheme();
    if (globalColorScheme == Qt::ColorScheme::Dark) {
        this->setStyleSheet(
...
/*
    ...
    connect(InputBox1ShortcutBackSpace, &QShortcut::activated, this, [this]() { m_currentSearchGeneration++; m_abstractModel->clear(); m_LineEdit2->clear(); m_LineEdit1->clear(); setWindowTitle(QDir::toNativeSeparators(m_currentDirectory)); m_LineEdit1->setFocus(); });
    ...
    connect(InputBox2ShortcutBackSpace, &QShortcut::activated, this, [this]() { m_currentSearchGeneration++; m_abstractModel->clear(); m_LineEdit2->clear(); m_LineEdit1->clear(); setWindowTitle(QDir::toNativeSeparators(m_currentDirectory)); m_LineEdit1->setFocus(); });
*/
...
void MainWindow::onCheckboxClickedCRC(Qt::CheckState state) {
    m_tableView->setColumnHidden(eColCRC, !state);
...
void MainWindow::onSearchFinished(uint iItemsFound, uint iNameMatched, uint iContentMatched, bool bSearchInterrupted) {
    qDebug() << "finalizeUI() entry point. m_BenchmarkTimer:" << m_BenchmarkTimer.elapsed() << " ms elapsed since start of search.  bSearchInterrupted =" << bSearchInterrupted;

    m_tableView->setRootIndex(QModelIndex());
    m_listView->setRootIndex(QModelIndex());
    m_thumbnailView->setRootIndex(QModelIndex());

    if (iItemsFound == 0 || bSearchInterrupted == true) {
        m_abstractModel->clear();
        m_currentSearchGeneration++; // Make old crc calc threads invalid to prevent seg faults from Use-After-Free
    }

    if (iContentMatched > 0) {
        m_tableView->setColumnHidden(eColQuality, true);
        m_tableView->setColumnHidden(eColCount, false);
        m_tableView->sortByColumn(eColCount, Qt::DescendingOrder);
    } else {
        m_tableView->setColumnHidden(eColCount, true);
    ...
    if (bSearchInterrupted == true) {
        ...
        titleString = QString(tr("%1 (%2 hits in %3 items)")).arg(QDir::toNativeSeparators(m_currentDirectory), QLocale::system().toString(iNameMatched), QLocale::system().toString(iItemsFound));
    } else {
        titleString = QString(tr("%1 (%2 matches spread across %3 files of %4 searched)")).arg(QDir::toNativeSeparators(m_currentDirectory), QLocale::system().toString(iContentMatched), QLocale::system().toString(iNameMatched), QLocale::system().toString(iItemsFound));
    }
    ...
    if (activeView) {
        ...
        //activeView->scrollTo(firstProxyIndex, QAbstractItemView::EnsureVisible);
    }
...
void MainWindow::updateColumns() {
    if (m_bHeaderVisible) {
        ...
        m_tableView->horizontalHeader()->setSectionResizeMode(eColCount,   QHeaderView::ResizeToContents);
        //m_tableView->horizontalHeader()->setSectionResizeMode(eColCRC,   QHeaderView::ResizeToContents);
